...
{
  return logicGateText[GateIndex(type)].text;
}
...
int GetLogicBlockGateType(const CString& text)
{
  ...
  for (i=0; i<sizeof logicGateText/sizeof logicGateText[0]; i++)
  {
    if (text == logicGateText[i].text) return logicGateText[i].type;
  };
  return -1;
}
...
int InputIndex(char type)
{
  int i;
  for (i=0; i<sizeof logicInputText/sizeof logicInputText[0]; i++)
    if (type == logicInputText[i].type) return i;
  return i-1;
}
...
{
  return logicInputText[InputIndex(type)].text;
}
...
int GetLogicBlockInputType(const CString& text)
{
  ...
  for (i=0; i<sizeof logicInputText/sizeof logicInputText[0]; i++)
  {
    if (text == logicInputText[i].text) return logicInputText[i].type;
  };
  return -1;
}
...
int ActionIndex(char type)
{
  int i;
  for (i=0; i<sizeof logicActionText/sizeof logicActionText[0]; i++)
    if (type == logicActionText[i].type) return i;
  return i-1;
}



const char *GetLogicBlockActionTypeText(int type)
{
  return logicActionText[ActionIndex(type)].text;
}
...
int GetLogicBlockActionType(const CString& text)
{
  ...
  for (i=0; i<sizeof logicActionText/sizeof logicActionText[0]; i++)
  {
    if (text == logicActionText[i].text) return logicActionText[i].type;
  };
  return -1;
}
...
const char *GetLogicBlockActionConditionText(int type)
{
  switch(type)
  {
...
{
  if (text == "IfTrue") return 0;
  if (text == "IfFalse") return 1;
  if (text == "Always") return 2;
  return -1;
}



const char *GetLogicBlockChainConditionText(int i)
{
  if (i == 0) return "Supress";
  if (i == 1) return "Normal";
  if (i == 2) return "Conditional";
  return "";
}

int GetLogicBlockChainCondition(const CString& text)
{
  if (text == "Supress") return 0;
  if (text == "Normal") return 1;
  if (text == "Conditional") return 2;
  return -1;
}
...
BEGIN_MESSAGE_MAP(CLogicBlock, CDialog)
  ...
  ON_BN_CLICKED(IDC_LOGIC_BLOCK, OnLogicBlockQuestion)
	//}}AFX_MSG_MAP
  ON_BN_CLICKED(IDC_IfTrue_1, &CLogicBlock::OnBnClickedIftrue1)
END_MESSAGE_MAP()
...
void CLogicBlock::OnBnClickedIftrue1()
{
  // TODO: Add your control notification handler code here
}
