...
//*****************************************************************************
//    NAME: addFlowControlEvent
//
// PURPOSE: 
//
//*****************************************************************************
static void addFlowControlEvent(GameEvent *event, UINT& result)
{
  //ASSERT(event->IsKindOf(RUNTIME_CLASS(FLOW_CONTROL_EVENT_DATA)));
  FLOW_CONTROL_EVENT_DATA *data = (FLOW_CONTROL_EVENT_DATA*)(event);

  CFlowControl dlg(*data);
...
void CEventViewer::DoDataExchange(CDataExchange* pDX)
{
  if (!pDX->m_bSaveAndValidate)
  {
    if (m_CurrEvent != NULL)
    {
      if (m_CurrEvent->GetEventControl().eventTrigger == QuestStageEqual)
      {
        m_questStage = m_PartyX;
        if ((m_questStage > 65001) || (m_questStage < 0)) m_questStage = 0;
      };
      if (m_CurrEvent->GetEventControl().eventTrigger == QuestStageNotEqual)
      {
        m_questStage = m_PartyX;
        if ((m_questStage > 65001) || (m_questStage < 0)) m_questStage = 0;
      };
  ...
  CDialog::DoDataExchange(pDX);
	...
	DDX_Control(pDX, IDC_ATTRIBUTES, m_Attributes);
	DDX_Text(pDX, IDC_QUESTSTAGE, m_questStage);
	DDV_MinMaxInt(pDX, m_questStage, 0, 65001);
	//}}AFX_DATA_MAP
  if (pDX->m_bSaveAndValidate)
  {
    if (m_CurrEvent != NULL)
...
BEGIN_MESSAGE_MAP(CEventViewer, CDialog)
    ...
    ON_BN_CLICKED(IDC_ATTRIBUTES, &CEventViewer::OnBnClickedAttributes)
	ON_BN_CLICKED(IDC_QUESTSTAGE, &CEventViewer::OnBnClickedQuestStage)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
UINT CEventViewer::EditEvent(GameEvent *event)
{
  ...
  case JournalEvent:
    ...
    break;
  case FlowControl:
    addFlowControlEvent(event, result);
    break;
  case PickOneCombat:
  //case InnEvent:    
  //case TavernTales:
...
void CEventViewer::UpdateControls(GameEvent *pEvent)
{
  ...
  m_questStage = pEvent->GetEventControl().partyX;
  if ((m_questStage > 65001) || (m_questStage < 0))
  {
    m_questStage = 0;
  };
...
void CEventViewer::SetControlVisibility(int id, bool show)
{
  CWnd* pWnd;
  pWnd = GetDlgItem(id);
  if (pWnd != NULL)
  {
    ...
    pWnd->ShowWindow(show ? SW_RESTORE : SW_HIDE);
  };
}
...
{
  HideControls(IDC_ItemTriggerTag, IDC_TRIGGERITEM);
  HideControls(IDC_TRIGGERSPECITEM, IDC_SpItemTriggerTag);
  HideControls(IDC_TRIGGERSPECKEY, IDC_SpKeyTriggerTag);
  HideControls(IDC_QUESTTRIGGER, IDC_QuestTriggerTag);


  //m_NPCTrigger.EnableWindow(FALSE);
  HideControls(IDC_NPCTRIGGER, IDC_NPCTriggerTag);
  //m_Race.EnableWindow(FALSE);
  HideControls(IDC_CHOOSERACE, IDC_RaceTag);
  //m_Facing.EnableWindow(FALSE);
  HideControls(IDC_CHOOSEFACING, IDC_FacingTag);
  //m_Class.EnableWindow(FALSE);  
  HideControls(IDC_CHOOSECLASS, IDC_ClassTag);
  //m_Gender.EnableWindow(FALSE);
  HideControls(IDC_CHOOSEGENDER, IDC_GenderTag);
  //m_Delete.EnableWindow(FALSE);
  HideControl(IDC_DELETEEVENT);
  //m_Insert.EnableWindow(FALSE);
  HideControl(IDC_INSERTEVENT);
  //m_EventType.EnableWindow(TRUE);
  ShowControl(IDC_EVENT_TYPE);
  //m_ConfigStepEvents.EnableWindow(FALSE);
  HideControl(IDC_CONFIGSTEPEVENTS);
  //m_ConfigZones.EnableWindow(FALSE);   
  HideControl(IDC_CONFIGZONES);
  //m_GPDLTrigger.EnableWindow(FALSE);
  HideControl(IDC_GPDLTRIGGER);
  //SpellTrigger.EnableWindow(FALSE);
  HideControls(IDC_SPELLTRIGGER, IDC_SpellTriggerTag);
  //m_Attributes.EnableWindow(m_CurrEvent != NULL);
  HideControl(IDC_ATTRIBUTES);
  //if (CopyEventData.GetCount() > 0)
    //   m_Paste.EnableWindow(TRUE);
  //else
    //   m_Paste.EnableWindow(FALSE);
  SetControlVisibility(IDC_PASTEEVENTS, CopyEventData.GetCount() > 0);
  //CWnd *pChanceWnd = GetDlgItem(IDC_PCNTCHANCE);
  // if (pChanceWnd != NULL)
  //    pChanceWnd->EnableWindow(FALSE);
  HideControls(IDC_PCNTCHANCE, IDC_PChanceTag);

  //CWnd *pPartyXWnd = GetDlgItem(IDC_PARTYXTRIGGER);
  // if (pPartyXWnd != NULL)
  //   pPartyXWnd->EnableWindow(FALSE);
  HideControls(IDC_PARTYXTRIGGER, IDC_PartyXTag);
  // CWnd *pPartyYWnd = GetDlgItem(IDC_PARTYYTRIGGER);
  // if (pPartyYWnd != NULL)
  //   pPartyYWnd->EnableWindow(FALSE);
  HideControls(IDC_PARTYYTRIGGER, IDC_PartyYTag);

   //CWnd* pQuestStageWnd = GetDlgItem(IDC_QUESTSTAGE);
   //if (pQuestStageWnd != NULL)
	 //  pQuestStageWnd->EnableWindow(FALSE);
  HideControls(IDC_QUESTSTAGE, IDC_QuestStageTag);
  HideControl(IDC_QuestStageDoc);
  m_Edit.SetWindowText("Add");
   ...
   case IS_REST_EVENT_ROOT:
   case IS_SINGLE_EVENT_ROOT:
     HideControl(IDC_EDITEVENT);
     HideControl(IDC_PASTEEVENTS);
     //m_Edit.EnableWindow(FALSE);
     //m_Paste.EnableWindow(FALSE);
     break;
   case IS_TIME_EVENT:
     //m_Edit.EnableWindow(TRUE);
     ShowControl(IDC_EDITEVENT);
     //m_ConfigStepEvents.EnableWindow(TRUE);
     ShowControl(IDC_CONFIGSTEPEVENTS);
     break;
   case IS_STEP_EVENT:
     //m_Edit.EnableWindow(TRUE);
     ShowControl(IDC_EDITEVENT);
   ...
   case RandomChance:
     ...
     ShowControls(IDC_PCNTCHANCE, IDC_PChanceTag);
     break;
   ...
   case PartyAtXY:
     ...
     ShowControls(IDC_PARTYXTRIGGER, IDC_PartyXTag);
     ShowControls(IDC_PARTYYTRIGGER, IDC_PartyYTag);
     break;

   case PartyHaveItem:
   case PartyNotHaveItem:
      //m_ItemTrigger.EnableWindow(TRUE);
     ShowControls(IDC_TRIGGERITEM, IDC_ItemTriggerTag);
      m_CurrEvent->GetEventControl().chance = 100;
      m_CurrEvent->GetEventControl().partyX = 0;
      m_CurrEvent->GetEventControl().partyY = 0;
      break;
   ...
   case PartyNotHaveSpecialItem:
      ...
      ShowControls(IDC_TRIGGERSPECITEM, IDC_SpItemTriggerTag);
      m_CurrEvent->GetEventControl().chance = 100;
      m_CurrEvent->GetEventControl().partyX = 0;
      m_CurrEvent->GetEventControl().partyY = 0;
      break;
   ...
   case PartyNotHaveSpecialKey:
      ...
      ShowControls(IDC_TRIGGERSPECKEY, IDC_SpKeyTriggerTag);
      m_CurrEvent->GetEventControl().chance = 100;
      m_CurrEvent->GetEventControl().partyX = 0;
      m_CurrEvent->GetEventControl().partyY = 0;
      break;
   ...
   case FacingDirectionAnyTime:
      ...
      ShowControls(IDC_CHOOSEFACING, IDC_FacingTag);
      break;
   ...
   case QuestNotPresent:
      ...
      ShowControls(IDC_QUESTTRIGGER, IDC_QuestTriggerTag);
      break;
   ...
   case QuestStageNotEqual:
	   ...
	   m_CurrEvent->GetEventControl().chance = 100;
	   m_CurrEvent->GetEventControl().partyY = 0;
     ShowControls(IDC_QUESTSTAGE, IDC_QuestStageTag);
     ShowControls(IDC_QUESTTRIGGER, IDC_QuestTriggerTag);
	   break;
   ...
   case NPCNotInParty:
      ...
      ShowControls(IDC_NPCTRIGGER, IDC_NPCTriggerTag);
      break;
   ...
   case ClassNotInParty:
      ...
      m_CurrEvent->GetEventControl().partyY = 0;
      {
        CWnd *x = GetDlgItem(IDC_Class_Baseclass);
        x->SetWindowText("Class");
        x = GetDlgItem(IDC_ClassTag);
        x->SetWindowText("Class");
      };
      //m_Class.EnableWindow(TRUE);
      ShowControls(IDC_CHOOSECLASS, IDC_ClassTag);
      break;
   ...
   case RaceNotInParty:
      ...
      ShowControls(IDC_CHOOSERACE, IDC_RaceTag);
      break;
   ...
   case GenderNotInParty:
      ...
      ShowControls(IDC_CHOOSEGENDER, IDC_GenderTag);
     break;
   ...
   case SpellMemorized:
     ...
     ShowControls(IDC_SPELLTRIGGER, IDC_SpellTriggerTag);
     break;
   ...
   {
     char name[MAX_QUESTTRIGGERNAME_TEXT + 1];
     if (m_CurrEvent->GetEventControl().quest >= 0)
       getBaseName(globalData.questData.GetName(m_CurrEvent->GetEventControl().quest), name, MAX_QUESTTRIGGERNAME_TEXT);
     else
...
HTREEITEM CEventViewer::FindItemFromString(HTREEITEM hItem, LPCTSTR szItemName, WORD type)
{
    ...
    {     
        ...
        if (MatchEventTextInSearch(type))
        {
          ...
          if (pEvent != NULL)
          {
            if (IsSearchTextAMatch(srchval, pEvent->text, type))
              return hItem;
            if (IsSearchTextAMatch(srchval, pEvent->text2, type))
              return hItem;
            if (IsSearchTextAMatch(srchval, pEvent->text3, type))
              return hItem;
...
void CEventViewer::OnBnClickedAttributes()
{
  if (m_CurrEvent == NULL) return;
  CEditAttributes dlg(m_CurrEvent->control.eventcontrol_asl, "");
  if (dlg.DoModal() == IDOK)
    dlg.GetData(m_CurrEvent->control.eventcontrol_asl);
}


void CEventViewer::OnBnClickedQuestStage()
{
