...
CHARACTER.prototype.buyItem = function (itemID, type) {
    ...
    if (this.GetAdjStatus() != charStatusType.Okay) {
        switch (this.GetAdjStatus()) {
            case charStatusType.Unconscious: Globals.SetMiscError(miscErrorType.UnconsciousChar); break;
            case charStatusType.Dead: Globals.SetMiscError(miscErrorType.DeadChar); break;
            case charStatusType.Fled: Globals.SetMiscError(miscErrorType.FledChar); break;
            case charStatusType.Petrified: Globals.SetMiscError(miscErrorType.PetrifiedChar); break;
            case charStatusType.Gone: Globals.SetMiscError(miscErrorType.GoneChar); break;
            case charStatusType.Animated: Globals.SetMiscError(miscErrorType.AnimatedChar); break;
            case charStatusType.TempGone: Globals.SetMiscError(miscErrorType.TempGoneChar); break;
            case charStatusType.Running: Globals.SetMiscError(miscErrorType.RunningChar); break;
            case charStatusType.Dying: Globals.SetMiscError(miscErrorType.DyingChar); break;
            default: Globals.SetMiscError(miscErrorType.UnknownError); break;
        }
        return;
    }
    ...
    if (!this.enoughMoney(cost, 0, 0, globalData.moneyData.GetDefaultType())) {
        Globals.SetMiscError(miscErrorType.NotEnoughMoney);
        return;
    }
    ...
    if ((itemData.getItemEncumbrance(itemID, 1) + this.GetEncumbrance()) > this.GetAdjMaxEncumbrance())
        ...
        if (!this.addCharacterItem(itemID, bundle, data.Num_Charges, true, cost))
            Globals.SetMiscError(miscErrorType.MaxItemsReached);
        else
...
CHARACTER.prototype.ReadyBestWpn = function (dist, isLargeTarget) {
    ...
    if (itemData.WpnUsesAmmo(this.myItems.GetItem(IdxToUse).itemID))
...
CHARACTER.prototype.GetAdjSize = function (flags) {
    ...
    if (this.GetAdjSpecAb(SPECAB.SA_Enlarge).returnVal)
        val++;
    if (this.GetAdjSpecAb(SPECAB.SA_Reduce).returnVal)
        val--;
...
CHARACTER.prototype.SetHitPoints = function (val) {
    ...
    if ((val > 0) && (val > this.hitPoints)) {
        ...
        if (this.GetAdjSpecAb(SA_Diseased).returnVal) {
            if (val > 1)
                val = 1;
        }
    }
...
CHARACTER.prototype.IsAnimal = function () {
    if (this.GetType() == MONSTER_TYPE) {
        return ((monsterData.GetMonsterFormFlags(this.monsterID) & MonsterFormType.FormAnimal) == MonsterFormType.FormAnimal);
    }
